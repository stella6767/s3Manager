...
class S3Controller(
    ...
    fun connect(
        ...
        val config = S3Config(region, bucket, accessKey, secretKey)

        // 연결 테스트
        val connectionResult = s3Service.testConnection(region, bucket, accessKey, secretKey)

        // 세션에 저장
        session.setAttribute("s3Config", config)
...
//
//        val result = s3Service.listObjectsPaginated(config, prefix, page, size)
//        if (!result.isSuccess) {
...
//            "components/s3/objectList"
//        } else {
//            "components/s3/s3Browser"
//        }
//    }
//
//    @GetMapping("/search")
